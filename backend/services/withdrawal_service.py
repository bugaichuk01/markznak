"""Сервис вывода КМ из оборота через True API (LK_RECEIPT)."""
...
from models import WithdrawalReport, WithdrawalStatus
...
async def list_withdrawal_reports(db: AsyncSession) -> list[WithdrawalReport]:
    result = await db.scalars(
        select(WithdrawalReport).order_by(WithdrawalReport.created_at.desc())
    )
